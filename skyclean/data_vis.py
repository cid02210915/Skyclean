...
class Visualise(): 
    def __init__(self, frequencies: list, realisation: int, lmax: int, directory: str = "data/"):
        ...
        self.saved_directories = {
            ...
            "ilc_synthesised": os.path.join(self.directory, "ILC_synthesised_maps"),
        }
    ...
    def visualise_maps(self, file_templates):
        """
        ...
        for i,(comp,file_template) in enumerate(file_templates.items()):
            for j, freq in enumerate(frequencies):
                fp = file_template.format(freq=freq, lmax = lmax, realisation = realisation)
    ...
    def visualise_wavelet_maps(self, file_templates):
        """
    ...
    def visualise_power_spectra(self, file_templates, ilc_output_path=None):
        """
        ...
        Parameters:
            ...
            realisation (int): The realisation number to visualise.
            desired_lmax (int): Maximum multipole for the power spectrum.
            file_templates (dict): Mapping from component name to a Python format-string for its filename.
        ...
        for i, freq in enumerate(frequencies):
            ...
            for comp, template in file_templates.items():
                ...
                if comp == 'ilc':
                    ...
                    cl = compute_and_save_ilc_power_spec(fp, ilc_output_path, realisation, desired_lmax)    
                else:
                    hp_map, _ = hp.read_map(fp, h=True)
                    hp_map*=1E6
                    cl  = hp.sphtfunc.anafast(hp_map, lmax=desired_lmax)
                    cl /= hp.pixwin(hp.get_nside(hp_map), lmax=desired_lmax)**2  # correct for pixel window function
    ...
    def visualise_component_ratio_power_spectra(self, file_templates, comp_a, comp_b, ilc_output_path=None):
        """
        ...
        for idx, freq in enumerate(frequencies):
            ...
            if comp_a == 'ilc':
                # load ILC only once at the first frequency
                cl_a = compute_and_save_ilc_power_spec(fp_a, ilc_output_path, realisation, desired_lmax)
            else:
                map_a = hp.read_map(fp_a)*1E6
                cl_a = hp.sphtfunc.anafast(map_a, lmax=desired_lmax)
            ...
            if comp_b == 'ilc':
                cl_b = compute_and_save_ilc_power_spec(fp_b, ilc_output_path, realisation, desired_lmax)
            else:
                map_b = hp.read_map(fp_b, verbose=False)*1E6
                cl_b = hp.sphtfunc.anafast(map_b, lmax=desired_lmax)
            # --- ratio and plot ---
            ratio = cl_a / cl_b
    ...
    def visualise_freq_ratio_spectra(components, freq1, freq2, file_templates):
        """
        """
        realisation = self.realisation
        desired_lmax = self.lmax
        ...
        for idx, comp in enumerate(components):
            ...
            cl1 = hp.sphtfunc.anafast(map1, lmax=desired_lmax)
            ...
            cl2 = hp.sphtfunc.anafast(map2, lmax=desired_lmax)
    ...
    def compute_and_save_ilc_power_spec(fp, 
                                        ilc_output_path, 
                                        realisation, 
                                        desired_lmax):
        # Recommended to run this first if computing ilc
        if os.path.exists(ilc_output_path.format(realisation=realisation)):
            cl = np.load(ilc_output_path.format(realisation=realisation))
        else:
            print(f"Existing ILC power spec not found at {ilc_output_path.format(realisation=realisation)}, computing now...")
            ilc_mw = np.load(fp)
            hp_ilc = MW_to_hp_map(ilc_mw)*1E6  #
            cl = hp.sphtfunc.anafast(hp_ilc, lmax=desired_lmax)
            cl /= hp.pixwin(hp.get_nside(hp_ilc), lmax=desired_lmax)**2
            np.save(ilc_output_path.format(realisation=realisation), cl)
        return cl
